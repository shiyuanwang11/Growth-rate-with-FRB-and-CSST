...
def D(sigma8,Om0,w,z):
    """
    the growth factou D(a), the range of z is [1e-3, 1]
    with the Initial condition D(a=1e-3)=1e-3, D'(a=1e-3)=1
    """

    def model(y,x,Om0=Om0,w=w):
        dydx = y[1]
        c = Om0/x**4 + (1.+w)*(1.-Om0)*x**(-4.-3.*w)
        A = 3./2.*c/Ea(x,Om0,w)**2 - 3./x
        B = 3./2. * Om0/x**5 / Ea(x,Om0,w)**2
        d2ydx2 = A*y[1] + B*y[0]
        return [dydx, d2ydx2]
